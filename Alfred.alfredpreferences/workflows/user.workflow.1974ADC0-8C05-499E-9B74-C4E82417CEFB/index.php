...
class BaiduSearch{
    ...
    static protected function DoSearch($query)
    {
        ...
        $searchRes = self::getSearchContent(urlencode($query));  
    ...
    static protected function showAlfred($info,$query){
        ...
        if(!self::$curlError){
            if(!empty($info))
            {
                ...
                self::$w->result(self::getuid(),self::getargu(['copy'=>$query,'url'=>$argu['website']]),'好像什么也没搜到，按enter键去百度官网搜索试试',''. $query,self::geticon());
            }
        }
        else 
        {
